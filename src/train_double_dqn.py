...
from dqn_agent import DQNHyperParams
...
def evaluate_agent(
    agent: DoubleDQNAgent, seed: int, num_episodes: int
...
def train_one_seed(args: argparse.Namespace, seed: int) -> None:
    ...
    agent = DoubleDQNAgent(obs_dim=obs_dim, num_actions=num_actions, device=args.device, hparams=hparams)

    results_dir = ensure_dir(ROOT / "results" / "double_dqn" / f"seed_{seed}")
    ckpt_dir = ensure_dir(ROOT / "checkpoints" / "double_dqn" / f"seed_{seed}")
    ...
    pbar = tqdm(range(1, args.total_timesteps + 1), desc=f"Training Double DQN seed {seed}")
    ...
    for step in pbar:
        ...
        if step % args.checkpoint_every == 0:
            agent.save(ckpt_dir / f"double_dqn_step_{step}.pt")
        ...
        if step % args.eval_every == 0 and step >= args.learning_starts:
            ...
            if is_best_eval:
                ...
                agent.save(ckpt_dir / "double_dqn_best.pt")
    ...
    agent.save(ckpt_dir / "double_dqn_final.pt")
    ...
    save_json(
        results_dir / "train_summary.json",
        {
            ...
            "best_model_path": str(ckpt_dir / "double_dqn_best.pt") if best_eval_step >= 0 else None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Double DQN on the shared highway benchmark.")
