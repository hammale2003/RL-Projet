...
def evaluate_agent(agent: DQNAgent, seed: int, num_episodes: int) -> tuple[list[dict[str, float | int]], dict[str, float | int]]:
...
def train_one_seed(args: argparse.Namespace, seed: int) -> None:
    ...
    agent = DQNAgent(obs_dim=obs_dim, num_actions=num_actions, device=args.device, hparams=hparams)

    results_dir = ensure_dir(ROOT / "results" / "dqn" / f"seed_{seed}")
    ckpt_dir = ensure_dir(ROOT / "checkpoints" / "dqn" / f"seed_{seed}")
    ...
    pbar = tqdm(range(1, args.total_timesteps + 1), desc=f"Training DQN seed {seed}")
    ...
    for step in pbar:
        ...
        if step % args.checkpoint_every == 0:
            agent.save(ckpt_dir / f"dqn_step_{step}.pt")
        ...
        if step % args.eval_every == 0 and step >= args.learning_starts:
            ...
            if is_best_eval:
                ...
                agent.save(ckpt_dir / "dqn_best.pt")
    ...
    save_json(
        results_dir / "train_summary.json",
        {
            ...
            "best_model_path": str(ckpt_dir / "dqn_best.pt") if best_eval_step >= 0 else None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the custom DQN agent on the shared highway benchmark.")
